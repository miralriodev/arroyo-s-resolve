...
export async function syncProfile(userId, payload = {}) {
  try {
    ...
    const profile = await repo.ensureProfile(userId, { full_name: name || undefined, avatar_url, role: finalRole });
    ...
    if (
      ...
      try {
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS contact_email text;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS phone text;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS host_title text;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS host_bio text;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS languages text[] DEFAULT '{}'::text[];");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS years_experience integer DEFAULT 0;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS response_rate integer DEFAULT 0;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS response_time text;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS superhost boolean DEFAULT false;");
        await prisma.$executeRawUnsafe("ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS city text;");
        const sets = [];
        ...
        if (sets.length) {
          await prisma.$executeRawUnsafe(`UPDATE public.profiles SET ${sets.join(', ')} WHERE id = '${userId}'`);
        }
      } catch (e) {
    ...
    try {
      const fallback = await repo.getProfileById(userId);
